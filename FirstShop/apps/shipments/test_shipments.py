...
class ShipmentTestAPI(TestCase):
    ...
    def test_list(self):
        c = APIClient()
        response = self.c.get(self.test_url)
        self.assertContains(response, 'johnny', status_code=200)
    ...
    def test_create_shipment(self):
        for i in range(1, 5):
                ...
                response = self.client.get(f'{self.test_url}{str(order)}')
                self.assertContains(response, Order.objects.values_list('phone', flat=True).get(public_id=order), status_code=301)
                print(response.status_code)
